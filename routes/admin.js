...
router.get('/stats', async (req, res) => {
  try {
    ...
    let userCount = 0;
    try {
      const [r] = await db.execute('SELECT COUNT(*) as c FROM users WHERE Email IS NOT NULL');
      userCount = r[0].c;
    } catch(e) {}
    let histCount = 0, sentTotal = 0;
    try {
      const [h] = await db.execute('SELECT COUNT(*) as c FROM ma_notification_history');
      const [s] = await db.execute('SELECT COALESCE(SUM(sent_count),0) as c FROM ma_notification_history');
      histCount = h[0].c; sentTotal = s[0].c;
    } catch(e) {}
    return res.json({
      total_users: userCount, subscribed: subRows[0].c,
      not_subscribed: userCount - subRows[0].c,
      notifications_sent: histCount, total_pushes_sent: sentTotal,
    });
  } catch (err) {
...
router.post('/schedule', async (req, res) => {
  try {
    ...
    if (day === undefined || hour === undefined) return res.status(400).json({ error: 'day and hour required' });
  ...
  try {
    ...
    if (Array.isArray(emails) && emails.length > 0) {
      ...
      [rows] = await db.execute(
        'SELECT endpoint,p256dh,auth_key,user_email FROM ma_push_subscriptions WHERE user_email IN ('+ph+')', emails
      );
    } else {
    ...
    for (const sub of rows) {
      try {
        await webpush.sendNotification(
          { endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth_key } }, payload
        );
        sent++;
      } catch (err) {
